...
4. IF no handler is registered for a dispatched Command type, THEN THE Mediator SHALL throw an InvalidOperationException identifying the Command type that has no registered handler
...
4. IF the EventPayload type does not map to any known Command type, THEN THE Event_Dispatcher SHALL publish a Notification representing an unrecognised event and log a warning via the Serilog_Pipeline identifying the payload type
...
1. THE Bot_Host SHALL register a logging Pipeline_Behavior that logs the Command type name and a unique request identifier before handler execution and logs the Command type name, request identifier, and elapsed time after handler execution via the Serilog_Pipeline
...
4. WHEN the Bot_Host starts, THE DI_Container SHALL scan designated assemblies and register all classes inheriting SagaStateMachine<TInstance> as state machine definitions
...
7. IF a Conversation_Event triggers a transition that is not defined for the current state, THEN THE Conversation_State_Machine SHALL remain in the current state and log a warning via the Serilog_Pipeline identifying the event type, current state, and correlation key
...
3. THE Event_Dispatcher SHALL publish a Conversation_Event via the MassTransit Bus indicating a new message received before dispatching the Command through the Mediator, so that the state machine transitions to Processing before handler execution begins
...
4. WHEN a Pipeline_Behavior or state machine transition logs a warning or error, THE Serilog_Pipeline SHALL include the trace identifier and span identifier in the structured log entry for correlation
...
4. WHEN a developer runs `dotnet test`, THE test project SHALL execute mediator and saga tests without requiring network access, Docker, or any external message broker
